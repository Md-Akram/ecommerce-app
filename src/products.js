export const Products = [
        {
                id: 1,
                name: "Apple iPhone 12",
                description: "The latest iPhone model featuring a 6.1-inch Super Retina XDR display, 5G capabilities, and A14 Bionic chip.",
                price: "799.99",
                imageUrl: "https://www.91-img.com/gallery_images_uploads/3/d/3df5ca6a9b470f715b085991144a5b76e70da975.JPG?tr=h-550,w-0,c-at_max"
        },
        {
                id: 2,
                name: "Samsung Galaxy S21 Ultra",
                description: "The premium Samsung smartphone with a 6.8-inch AMOLED 2X display, 5G capabilities, and a powerful Exynos 2100 processor.",
                price: "1199.99",
                imageUrl: "https://cdn.shopify.com/s/files/1/1706/9177/products/samsung-galaxy-s21-ultra-g998-5g-128gb-dual-sim-negro_540x.jpg?v=1619686227"
        },
        {
                id: 3,
                name: "Google Pixel 5",
                description: "The latest Google smartphone with a 6-inch FHD+ OLED display, 5G capabilities, and a Snapdragon 765G processor.",
                price: "699.99",
                imageUrl: 'https://fdn2.gsmarena.com/vv/bigpic/google-pixel-5-5g.jpg'
        },
        {
                id: 4,
                name: "Apple MacBook Pro",
                description: "The latest MacBook Pro model featuring a 13.3-inch Retina display, 8th-generation Intel Core i5 processor, and Touch Bar.",
                price: "1299.99",
                imageUrl: 'https://mcsolution.com.bd/wp-content/uploads/2022/06/macbook-pro-m2-macbook-pro-price-in-bd.webp'
        },
        {
                id: 5,
                name: "Microsoft Surface Laptop 4",
                description: "The latest Surface Laptop model featuring a 13.5-inch PixelSense display, 11th-generation Intel Core i5 processor, and Windows 10 operating system.",
                price: "999.99",
                imageUrl: 'https://cdn.shopify.com/s/files/1/1706/9177/products/Microsoft-Surface-Laptop4-CustommacBD_1.jpg?v=1632394945'
        },
        {
                id: 6,
                name: "Dell XPS 13",
                description: "The latest Dell XPS 13 model featuring a 13.3-inch FHD+ InfinityEdge display, 11th-generation Intel Core i7 processor, and Windows 10 operating system.",
                price: "1199.99",
                imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3N77gnz6UCgMB81ERmsZUZ3VsGto5HVxttw&usqp=CAU'
        },
        {
                id: 7,
                name: "Bose QuietComfort 35 II Wireless Headphones",
                description: "Wireless noise-cancelling headphones with a comfortable, over-ear design and Alexa voice control.",
                price: "349.99",
                imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNW5AiaUHHvYsiBPSFNbaN9UKB6qctswQlBw&usqp=CAU"
        },
        {
                id: 8,
                name: "Beats by Dr. Dre Powerbeats Pro Wireless Earphones",
                description: "Wireless earphones with a sweat and water-resistant design, up to 9 hours of listening time, and hands-free control with Siri.",
                price: "249.99",
                imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTs8wbSJ2IjO-xBTG4cQE1DZAZ7IR8fmIQPfA&usqp=CAU'
        },
        {
                id: 9,
                name: "Sonos One SL Smart Speaker",
                description: "A compact, voice-controlled smart speaker with a sleek design and powerful sound quality.",
                price: "179.99",
                imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSh4zTWPOJ_guwU2HODZf-po4go5wqmdasjRg&usqp=CAU'
        },
        {
                id: 10,
                name: "LG OLED CX Series Smart TV",
                description: "A 55-inch 4K smart TV with an OLED display, ThinQ AI technology, and support for popular streaming apps.",
                price: "1799.99",
                imageUrl: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR10t38AmzmoiRemOuTIHmZtDNEhvibOlnTmg&usqp=CAU'
        }
]